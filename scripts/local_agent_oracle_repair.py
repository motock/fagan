"""Syntax/undefined-name repair and detection helpers for scripts/local_agent_
oracle.py's run_tool. Split out purely to keep local_agent_oracle.py under the
project's line-count target; the content here was ported verbatim from
scripts/local_agent.py originally and is kept in its own docstrings' wording
(this module mirrors scripts/local_agent_repair.py, local_agent.py's own
equivalent split, but the two are independently maintained, near-duplicate
files, not a shared import - see reference_benchmark_harness_gotchas).

_SYNTAX_REJECT_COUNTS is shared mutable state with local_agent_oracle.py's
run_tool (which resets a path's count on a successful write) - defined here
and re-imported into local_agent_oracle.py so both modules mutate the SAME
dict object; unlike a monkeypatched function, a shared mutable container
stays in sync across the import regardless of which module's name is used to
reach it.
"""
import ast
import subprocess
...
def _lint_feedback_for(path_str: str, cwd: Path) -> str:
    ...
    test's monkeypatch of local_agent_oracle.CWD - a module-level import
    here would freeze the value at this module's own load time instead."""
    if not path_str.endswith(".py"):
        return ""
