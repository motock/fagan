// URL hash deep-linking for the dashboard.
//
// Extracted verbatim from static/app.js. The hash encodes the plan + active
// filters so a view is shareable via URL and survives reload / browser
// back-forward. Defaults are omitted to keep URLs short. Unknown values are
// silently dropped (graceful fallback to defaults). localStorage remains a
// secondary store for when the URL has no hash at all (e.g. a fresh tab that
// previously set filters).
//
// routing.js depends only on state.js (acyclic layer: state ← routing).
...
export function hashStateFrom(s) {
  return {
