...
def _restore_wedge_config_module(monkeypatch):
    ...
    runs its teardown AFTER the test's own monkeypatch undo, so deleting the
    vars here also cleans up anything a test set, and the final reload leaves
    the module in its pristine default state for later tests.
    """
    yield
