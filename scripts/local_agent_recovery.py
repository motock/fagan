"""5xx oversized-transcript recovery, relocated from scripts/local_agent.py
(LA-RECOVERY). Behavior-preserving move: every la-owned name the body needs is
read through the caller's live module dict (`origin`), which
scripts/local_agent.py's delegating wrapper passes as its own globals().
"""
...
# `la.time.sleep`, which mutates the shared stdlib time module object; this
