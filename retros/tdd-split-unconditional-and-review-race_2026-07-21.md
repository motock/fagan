...
Mode 30's working-tree-mutating ops (`git merge --ff-only` in `_sync_local_default_branch`, `git pull --ff-only` in dispatch) were removed by the predecessor story; scheduler/dispatch no longer rewrite checked-out files. Dispatch now creates worktrees directly from `origin/<branch>` via `git fetch` + `git worktree add ... origin/<branch>`. The remaining `.git`-only write (the fetch) is guarded by a non-blocking advisory flock (`_try_acquire_git_lock` in `pipeline/server.py`) so concurrent human git operations are not raced. `pause_plan` before manual git surgery is now defense-in-depth, not load-bearing. See PRs #177 and #178.
