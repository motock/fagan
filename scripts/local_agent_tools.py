"""Tool-dispatch impls extracted from the local dispatch agent module (LA-TOOLS).

The two functions below were moved VERBATIM from the agent module
(run_tool, safe_run_tool), renamed to ``<name>_impl`` with ``origin`` added
...
Mutation contract: the per-run state containers are mutated IN PLACE through
origin (``origin["_CREATED_THIS_RUN"].add(...)``,
``origin["_VIEWED_THIS_RUN"].add(...)``,
``origin["_SYNTAX_REJECT_COUNTS"].pop(...)``) — never copied to a local first
— so the agent module's resident objects stay the single source of truth
across calls, and a test that REBINDS ``la._SYNTAX_REJECT_COUNTS`` to a fresh
dict is picked up by the very next call (the origin read happens per call, so
the fresh patch-dict is what gets mutated). ``safe_run_tool_impl`` dispatches
through ``origin["run_tool"]`` for the same late-binding reason: a test
patching ``la.run_tool`` with a fake must be observed by the moved
safe_run_tool, and re-patching must be seen by the next call.

This module imports ONLY the stdlib — it must never import the agent module,
its config module, or the repair module (the repair/guard helpers arrive via
``origin`` because the agent module imports them into its own namespace;
a direct import here would freeze one sys.modules instance and break
per-instance routing, besides risking import cycles).
"""
...
def run_tool_impl(origin, fn, args) -> str:
    ...
    if fn == "checkpoint":
        try:
            res = origin["p"]._checkpoint_impl(args["plan_name"], args["story_key"], args["step"],
...
def safe_run_tool_impl(origin, fn, args) -> str:
    ...
    try:
        ...
        required = origin["_TOOL_SCHEMAS"].get(fn, {}).get("required", [])
