"""Hidden acceptance oracle for the ratelimiter_inspect task.
...
def test_available_tokens_reflects_refill_capped_at_capacity():
    b = TokenBucket(5, 100, now=0.0)
    assert b.allow(5, now=0.0) is True
    assert b.available_tokens(now=10.0) == 5  # refill capped, not overshot


def test_available_tokens_does_not_mutate_state():
    """The core mutation-safety requirement: calling the read-only inspector,
    even repeatedly, must never change what allow() sees afterwards."""
    b = TokenBucket(3, 1, now=0.0)
    for _ in range(5):
        b.available_tokens(now=0.0)
    assert b.allow(3, now=0.0) is True  # still full capacity available


def test_available_tokens_after_inspection_then_allow_matches():
    b = TokenBucket(2, 1, now=0.0)
    assert b.allow(2, now=0.0) is True  # drain to 0
    assert b.available_tokens(now=1.0) == 1  # 1s * 1 token/s refilled
    assert b.allow(1, now=1.0) is True
    assert b.allow(1, now=1.0) is False  # drained again, no leftover from inspection
