...
``pipeline/server.py`` (a behavior-preserving file move). The classes read
several module-level names as free variables so the test suite's
``monkeypatch.setattr(pipeline.server, ...)`` targets keep landing exactly as
they did before the extraction. ``PLAN_DIR`` and ``WORKTREE_ROOT`` are resolved
lazily via the module ``__getattr__`` below so a patch on ``pipeline.server``
is observed at call time rather than frozen at import time.
"""
...
def __getattr__(name: str):
    """Resolve server-sourced globals lazily so monkeypatches on
    ``pipeline.server`` are observed at call time."""
    import pipeline.server as _s
...
class FileStore:
    ...
    def manifest_path(self, plan_name: str) -> Path:
        return PLAN_DIR / f"{plan_name}.manifest.json"  # noqa: F821
    ...
    list_plans = lambda self: [f.stem for f in PLAN_DIR.glob("*.json")]  # noqa: F821

    def list_manifests(self) -> list[str]:
        return [
            mp.name.removesuffix(".manifest.json")
            for mp in sorted(PLAN_DIR.glob("*.manifest.json"))  # noqa: F821
        ]
    ...
    def get_notifications(self, plan_name: str) -> list[str]:
        ...
        path = PLAN_DIR / f"{plan_name}.notifications.log"  # noqa: F821
        if not path.exists():
            return []
    ...
    def get_notification_records(self, plan_name: str, limit: int = 100) -> list[dict]:
        ...
        if limit <= 0:
            limit = 100
        path = PLAN_DIR / f"{plan_name}.notifications.jsonl"  # noqa: F821
        if not path.exists():
            return []
        records: list[dict] = []
    ...
    def get_decisions(self, plan_name: str) -> list[dict]:
        ...
        path = PLAN_DIR / f"{plan_name}.decisions.json"  # noqa: F821
        if not path.exists():
            return []
        try:
    ...
    def get_journal(self, plan_name: str, story_key: str) -> tuple[bool, list[dict]]:
        ...
        path = PLAN_DIR / f"{plan_name}.{story_key}.journal.json"  # noqa: F821
        if not path.exists():
            return False, []
        try:
    ...
    def get_story_log(self, plan_name: str, story_key: str, manifest: dict[str, Any], lines: int = 200) -> dict[str, Any]:
        ...
        if log_path.is_absolute():
            ...
            # would be unsafe).
            return empty
        log_path = PLAN_DIR / raw_log  # noqa: F821
        try:
            log_path = log_path.resolve(strict=False)
            plan_dir_resolved = PLAN_DIR.resolve()  # noqa: F821
    ...
    def get_worktree_file(self, story: dict[str, Any], filename: str) -> dict[str, Any]:
        ...
        try:
            ...
            root = WORKTREE_ROOT.resolve()  # noqa: F821
