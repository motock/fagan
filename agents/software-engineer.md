...
If the full suite surfaces failures in files or behavior your change did not touch, treat them as pre-existing and out of scope: do not investigate or attempt to fix them, and do not spend further steps on them. Note them briefly in your final summary, then pause and surface the issue via `request_decision` with options such as: proceed treating them as pre‑existing and out of scope; investigate further; or park for review, and follow the overlord's ruling before proceeding.
...
tests pass, commit with a Conventional Commit message.