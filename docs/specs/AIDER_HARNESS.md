# Aider harness contract (aider-chat 0.86.2)

Captured 2026-09-07 from a throwaway `uv tool install aider-chat` install
(outside this repo; the repo venv and requirements files were never touched).
Every flag named in the sections below appears verbatim in the
`## Verbatim --help capture` block, which is the single source of truth for
this document.

## Verbatim --help capture

- Version (`aider --version`): `aider 0.86.2`
- Date captured: 2026-09-07 (`date -u +%Y-%m-%d`)
- Command: `aider --help > aider_help.txt 2>&1` (stderr included)
...
One row per flag the adapter will use. Every "capture line" is quoted verbatim
from the fenced block above; a reviewer can confirm each by eye.

| Purpose | Flag | Exact line(s) from the capture |
|---|---|---|
| Pass a single instruction and exit | `--message COMMAND` (aliases `--msg`, `-m`) | `--message COMMAND, --msg COMMAND, -m COMMAND` / `Specify a single message to send the LLM, process` / `reply then exit (disables chat mode) [env var:` / `AIDER_MESSAGE]` |
| Suppress ALL interactive confirmation prompts | `--yes-always` | `--yes-always          Always say yes to every confirmation [env var:` / `AIDER_YES_ALWAYS]` |
| Set the model | `--model MODEL` | `--model MODEL         Specify the model to use for the main chat [env var:` / `AIDER_MODEL]` |
| Set the working directory | **No flag exists.** Aider operates on the process cwd (like `claude -p`): the capture has no cwd/working-directory option, and the config-file line confirms cwd-relative discovery: `Specify the config file (default: search for` / `.aider.conf.yml in git root, cwd or home directory)`. The adapter must `os.chdir()` (or set `cwd=` on the subprocess) before exec. | (absence — see the full capture; no such flag appears) |
| Disable aider's own automatic git commits (the pipeline owns committing) | `--no-auto-commits` | `--auto-commits, --no-auto-commits` / `Enable/disable auto commit of LLM changes (default:` / `True) [env var: AIDER_AUTO_COMMITS]` |
| Disable commits when the repo is found dirty (second auto-commit path) | `--no-dirty-commits` | `--dirty-commits, --no-dirty-commits` / `Enable/disable commits when repo is found dirty` / `(default: True) [env var: AIDER_DIRTY_COMMITS]` |
| Disable pretty/colorized output so the log file stays parseable | `--no-pretty` | `--pretty, --no-pretty` / `Enable/disable pretty, colorized output (default:` / `True) [env var: AIDER_PRETTY]` |
| Disable streaming output so the log file stays parseable | `--no-stream` | `--stream, --no-stream` / `Enable/disable streaming responses (default: True)` / `[env var: AIDER_STREAM]` |
| Disable fancy terminal input handling (raw mode / completion UI) in the non-interactive child | `--no-fancy-input` | `--fancy-input, --no-fancy-input` / `Enable/disable fancy input with history and completion` / `(default: True) [env var: AIDER_FANCY_INPUT]` |

UNRESOLVED rows (cannot cite a capture line, so not asserted):

- Whether `--yes-always` alone suppresses *every* prompt class (e.g. the
  Playwright-install prompt has its own `--disable-playwright` switch, and
  git-identity prompts are not mentioned in the capture). The adapter should
  pass `--yes-always` and also run with stdin closed; treat residual prompts
  as a hang and kill on timeout. No capture line documents a stronger
  "never prompt" switch.
- Whether `--no-pretty` + `--no-stream` fully linearize output under a pipe.
  Observed probe output (see `## Exit behavior`) was plain text, but a
  success-path capture with a live key was not possible (no key in this
  environment), so the exact success-path log format is UNRESOLVED.

## Environment

Env vars the tool reads for upstream API keys (each cited from the capture):

- `AIDER_OPENAI_API_KEY` — capture line: `Specify the OpenAI API key [env var:` / `AIDER_OPENAI_API_KEY]` (the `--openai-api-key` option).
- `AIDER_ANTHROPIC_API_KEY` — capture line: `Specify the Anthropic API key [env var:` / `AIDER_ANTHROPIC_API_KEY]` (the `--anthropic-api-key` option).
- The probe run additionally confirmed the upstream default: with `gpt-4o` and
  no key set, aider/litellm failed with `The api_key client option must be set
  either by passing api_key to the client or by setting the OPENAI_API_KEY
  environment variable` — i.e. litellm also reads the provider-native
  `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` variables. The adapter should set the
  provider-native variables (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`) in the
  child process env; they are the ones litellm actually consults.
- `--api-key PROVIDER=KEY` exists (`Set an API key for a provider (eg: --api-key` / `provider=<key> sets PROVIDER_API_KEY=<key>)`) but the adapter must NOT
  use it: it puts the key in argv.

Key-handling rules (mandatory):

- Keys must NEVER be echoed into argv. argv is world-readable for the lifetime
  of the process via `ps`/`ps -ef`, and is captured in shell history. Keys are
  passed only through the child process environment (`env=` on the subprocess),
  never as `--openai-api-key`/`--api-key`/`--set-env` values.
- Keys must never be written into the story log, the harness log file, or any
  error output that gets logged. Probe logs in this story were checked before
...
`f.txt` tracked; baseline `git rev-list --count HEAD` = 1, porcelain empty),
using only capture-cited flags:
`aider --model <M> --message 'x' --yes-always --no-auto-commits --no-dirty-commits --no-pretty --no-stream`.
No API key existed in the environment, so no success-path probe was possible.

| Case | Observed result | Exit code | Git state after (rev-list count / porcelain) |
|---|---|---|---|
| Unknown model string (`--model not-a-real-model-xyz`) | Startup banner, then `litellm.BadRequestError: LLM Provider NOT provided. Pass in the LLM provider you are trying to call. You passed model=not-a-real-model-xyz` | **0** | count still 1; porcelain gained only `?? .gitignore` (aider created it) plus the probe log. **No commit.** |
| Known model, required key missing (`--model gpt-4o`, `OPENAI_API_KEY` unset) | `litellm.AuthenticationError: AuthenticationError: OpenAIException - The api_key client option must be set either by passing api_key to the client or by setting the OPENAI_API_KEY environment variable` / `The API provider is not able to authenticate you. Check your API key.` | **0** | count still 1; porcelain unchanged apart from probe logs. **No commit.** |
| Provider unreachable / model absent (`--model ollama/qwen3:0.6b`, local Ollama reachable, model not pulled) | `litellm.APIConnectionError: OllamaException - {"error":"model 'qwen3:0.6b' not found"}` with retries at 8s/16s/32s backoff | **0** | count still 1. **No commit.** |
| Success (LLM replies, edits applied) | Not runnable here — requires a live API key | UNRESOLVED | UNRESOLVED |

Findings the adapter must encode:

- **Exit code 0 does not mean success.** Every failure mode probed — unknown
  model, missing key, provider error — exited 0. The adapter cannot gate on
  the return code; it must parse the log for `litellm.` error markers (or
  capture the assistant reply) and treat the run as failed when they appear.
- **`--no-auto-commits` held under failure.** No probe produced a commit
  (count stayed 1 in all three), so the pipeline's branch state is safe even
  when aider fails mid-run.
- **Aider writes files into the worktree as a side effect.** The first probe
  created an untracked `.gitignore` (aider adds `.aider*` entries; see capture
  line `Enable/disable adding .aider* to .gitignore (default:` / `True) [env var: AIDER_GITIGNORE]`). The adapter should expect `.aider*` artifacts in
  the worktree and not mistake them for agent output.
- Retry/backoff behavior on provider errors (8s/16s/32s) means a hung-looking
  run can be legitimate; the adapter needs a generous timeout rather than a
  fast kill.

## Fit against HarnessRequest

`app/harness.py` (lines 52-57) defines `HarnessRequest` with exactly these
fields, spelled as in code: `prompt`, `system`, `model`, `cwd`, `acceptance`,
`options`. Mapping each onto the resolved contract:

| HarnessRequest field | Aider mapping | Verdict |
|---|---|---|
| `prompt: str` | `--message '<prompt>'` — "Specify a single message to send the LLM, process reply then exit (disables chat mode)". | **Fits.** |
| `system: str \| None` | **NO equivalent.** The capture has no system-prompt flag. The closest things are `--show-prompts` (prints prompts, debug only) and `--read FILE` (injects a read-only file into the chat, which is not a system prompt and costs a file write). Aider's system prompts are fixed per edit-format. | **Gap.** The adapter must prepend the system text into `--message` (e.g. "System instructions: …\n\nTask: …") or drop it; there is no first-class channel. |
| `model: str` | `--model MODEL` — "Specify the model to use for the main chat". Note the value must be a litellm-style provider-qualified name (`ollama/...`, `gpt-4o`, …); a bare unknown string fails at call time with `litellm.BadRequestError` (exit 0). | **Fits**, with a naming-convention caveat the registry must own. |
| `cwd: str` | **No flag.** Aider uses the process cwd (config discovery is "git root, cwd or home directory"; no chdir option exists in the capture). The adapter must set the subprocess `cwd=` — same pattern as `claude -p`. | **Fits via subprocess cwd**, not via a tool flag. |
| `acceptance: list[str] \| None` | **NO equivalent.** Aider has `--test` / `--test-cmd` / `--auto-test` ("Run tests, fix problems found and then exit"), which is a single fixed test command aider runs itself — not a list of acceptance commands the pipeline grades, and it makes aider fix failures (mutating the worktree in unreviewed ways). | **Gap.** Acceptance must stay with the pipeline: run the acceptance commands after aider exits, on the worktree aider left. Do not map acceptance onto `--test`. |
| `options: dict \| None` | Partially expressible: harness-specific keys can map to capture-cited flags (`--no-auto-commits`, `--no-dirty-commits`, `--no-pretty`, `--no-stream`, `--no-fancy-input`, `--yes-always`, `--model`). Unknown keys are ignorable by contract ("a harness reads the keys it knows and ignores the rest"). | **Fits** for the flags above; any option key with no capture-cited flag must be rejected or ignored, never guessed into a flag. |

Gap summary (the deliverable of this epic): Aider's CLI covers `prompt`,
`model`, and (via subprocess cwd) `cwd`, and `options` is serviceable — but it
has **no equivalent for `system`** and **no equivalent for `acceptance`**.
Two of six `HarnessRequest` fields do not map onto Aider's contract. That is
concrete evidence that `app/harness.py`'s abstraction is not automatically
portable to a genuinely unrelated tool: an Aider adapter must fold `system`
into the prompt text and keep `acceptance` entirely outside the tool. Any
future harness story that assumes a first-class system prompt or pipeline-run
acceptance needs a documented workaround per tool, as done here.