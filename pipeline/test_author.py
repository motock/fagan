...
def _test_author_prompt(agent_instructions: str) -> str:
    ...
    return (
        ...
        "count: assert them STRUCTURALLY (read the file, assert the new name, "
        "key or row is present and the old one is gone) - never assert the "
        "exact wording of a docstring, comment or prose sentence, because the "
        "implementer then edits prose to match your wording instead of "
        "implementing the behavior. Anything you leave "
        ...
        "cannot be tested, say so explicitly in your final message. Keep the "
        "suite proportionate: one focused test per requirement plus its "
        "negative and boundary cases, no parametrized matrices over prose, and "
        "well under 200 lines for a change of a few dozen lines - an oversized "
        "suite spends the implementer's step budget before it finishes. "
        "Grade BEHAVIOR through the public API only: call the public "
        "functions, classes or commands the task describes and assert what "
        "they return or do. Never read implementation source text to grade "
        "it - not via include_str! and not by reading a source file and "
        "grepping it - and never pin a private field's or type's exact "
        "declaration, because both freeze an implementation detail that a "
        "later story is free to change. A scope constraint in the task "
        "(e.g. \"do NOT touch X in this story\") is an instruction for the "
        "implementer, never an assertion: a test that fails when a later "
        "story legitimately edits X cannot be fixed afterwards. "
        "EXCEPTION: "
