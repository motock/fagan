...
HEAD branch (the rework alias ``agent/<key>-<suffix>``), not the hardcoded
convention branch ``agent/<key>``.

Live incident (LA-VERIFY rework-alias follow-up, 2026-08-31): a rework round
leaves the story worktree checked out on an alias branch ``agent/s10-2``.
``_open_pr`` and ``_merge_pr`` already resolve that alias via
``pipeline/pr.py::_resolve_story_branch``, but the merge gate does not:

  - ``pipeline/merge.py::_rebase_and_push_for_merge`` (called from
    ``pipeline/advance.py``'s merge adjudication, which passes
    ``branch = f"agent/{key.lower()}"``) pushes
    ``git push --force-with-lease origin <convention branch>`` from REPO_ROOT,
    and advance.py CI-checks that same convention branch at the
    worktree-HEAD SHA - while ``_merge_pr`` merges the RESOLVED alias branch.
...
squash-merged and deleted by a prior ``_merge_pr`` (``git branch -D``), so the
gate's push failed with ``src refspec agent/s10 does not match any`` and the
story parked at the merge gate forever - the tests_passed loop became a
merge-gate loop, not a fix. If a stale local convention branch survived
instead, the gate pushed that stale code while ``_ci_status_once`` polled
``commits/<worktree-HEAD-sha>/check-runs`` - a SHA never pushed to the polled
branch - so CI stayed pending until expiry; and with
...
They fail against the current code because the gate pushes/CI-checks the
hardcoded convention branch; they pass once the gate resolves the worktree's
actual HEAD branch (``_resolve_story_branch``) so rebase -> push -> CI ->
_merge_pr all operate on one branch.
"""
...
# ---------------------------------------------------------------------------
# Real-git fixture helpers (self-contained; git itself is never mocked).
# ---------------------------------------------------------------------------
...
def _init_story_repo(tmp_path, *, delete_local_convention):
    """Create ``origin`` (bare) + a story repo with the incident topology.

    Leaves the repo HEAD on the alias branch ``agent/s10-2`` @ alias_sha with
    ``origin/agent/s10-2`` @ alias_sha, plus a STALE local ``agent/s10`` that
    is one unpushed commit ahead of ``origin/agent/s10`` - or, with
    ``delete_local_convention=True``, the local ``agent/s10`` branch deleted:
    exactly the state a prior ``_merge_pr`` ``git branch -D`` leaves behind.
...
def _call_rebase_and_push_for_merge(plan_name, key, branch, worktree):
    ...
    ``(plan_name, key, branch, worktree)`` signature - resolving the worktree
    HEAD internally and ignoring the passed convention branch - or drop the
    branch parameter entirely (advance.py stops passing it). Both shapes must
...
# ---------------------------------------------------------------------------
class TestAdvanceMergeGateResolvedAliasBranch:
...
# ---------------------------------------------------------------------------
class TestApproveMergeManualPathResolvedAliasBranch:
    def test_manual_path_pushes_and_ci_checks_resolved_alias_branch(
        self, plan_dir, tmp_path, monkeypatch,
    ):
        # The human-approved merge path must resolve the worktree's alias
        # HEAD for its --force-with-lease push and CI check too (same bug,
        # second site).
        repo, _conv_origin_sha, alias_sha = _init_story_repo(
            tmp_path, delete_local_convention=True
        )
        ...
        assert ci_calls, (
            "the manual merge path must CI-check the pushed branch"
        )
        assert ci_calls[0][0] == "agent/s10-2", (
