...
| `PIPELINE_REVIEW_INCONCLUSIVE_MAX` | `2` | Inconclusive-review budget: how many consecutive non-rate-limited `UNKNOWN` verdicts (no parseable `VERDICT` line, or the reviewer-exception fail-safe) are retried on later ticks — without counting against `PIPELINE_REWORK_MAX_ATTEMPTS` or touching `review_feedback` — before the story is `parked` for human review instead of retrying forever. A reviewer transport failure is not counted here: an `UNKNOWN` whose output still looks like a transient backend error (HTTP 5xx, bad gateway, service unavailable, connection reset or refused, timed out) after the single inline retry, or a timeout/connection exception raised by the reviewer backend, defers the review to the next tick like a rate limit (`review_deferred_count` increments; `review_inconclusive_count` does not) |
