...
alone. The executor's own tests are never the grade. This document specifies
five normative rules (§2), the outcome classification that detects unusable
and born-broken oracles before they burn an executor's budget (§3), and an
adoption checklist (§4). The key words MUST, SHOULD, and MAY are to be
...
2. **Self-consistent bugs.** A test that exists and passes is necessary but
   ...
   off-by-one, an inclusive/exclusive edge — can write a fully green suite
   that encodes the *same* mistake as its implementation. The test then
...
rewritten. Before any grading run, the harness MUST compare each fixture's
worktree bytes against its pinned digest. On mismatch it MUST refuse WITHOUT
running anything: a rewritten grader is not a trustworthy gate, and running
it is self-grading again — the exact failure the pattern exists to prevent.
The refusal is a tamper refusal, not a red fixture and not a partial run.
...
Acceptance fixtures bypass every quality role in the harness: no reviewer
persona sees them before dispatch. A fixture with a lint violation therefore
becomes a read-only oracle the executor is forbidden to touch, and a repo-wide
...
exercise the unit in isolation only, surfacing it as a prompt to re-check
the fixture rather than a hard gate.
...
regression check redundant. The harness MUST provide a backstop *independent*
of both the executor and the fixtures: a reviewer who diffs the delivered
change against the actual requirement (not against "did the tests pass"),
plus a full-suite run of the delivered branch — outside the executor's
control, since a full-suite run performed by the executor is still
self-grading. The layers have different coverage and neither substitutes for
the other: fixtures grade the requirement's core behavior; the reviewer
catches spec-completeness gaps no assertion covers; the full suite catches
regressions in behavior the fixtures never touch. A merge-time
re-verification of the fixtures against the just-rebased branch — independent
of both review and CI — SHOULD close the residual gap for repos without CI
and for slips between "tests passed" and merge.

## 3. Outcome classification and born-broken detection

### 3.1 Outcome states

Grading a fixture set against a checkout yields one of four states, and the
states demand opposite actions — the harness MUST classify before reacting:
...
MUST be treated as born-broken — the safe direction: a born-broken oracle
wastes whole dispatches, while a false "born-broken" verdict merely delays
one.
...
- **c4 (a later executor change):** a genuine regression breaks F. Because
  ...
  point: getting c2's classification right is not enough — the recorded
  baseline MUST still be truthful when the next call reads it. A c2 verdict
  produced by overwriting the baseline to green, rather than by classifying
  the failure as a prior-gate trip, masks the c4 regression against a
  false-green baseline, and the harness silently merges a regression it was
  built to catch. The verdict at c2 is only as good as the baseline state it
  leaves behind for c4.

## 4. Adoption checklist

A harness adopting this pattern MUST provide the following four provisions,
each stated with the minimum viable form; refinements are optional.
...
4. **Outcome classification.** Implement the four states of §3.1 and the
   ...
   `errors`; proceed only on `fails_correctly`; re-verify the fixtures at
