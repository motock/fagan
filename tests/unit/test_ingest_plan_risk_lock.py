...
def test_reingest_still_refreshes_other_authored_fields_on_dispatched_story(
    ...
    plan["epics"][0]["stories"][0].update({
        ...
        # Preflight: test fixture — not a real plan (the ollama backend makes
        # this ingest hit the local-dispatch preflight gate).
        "agent_instructions": (
            "Preflight: test fixture — not a real plan\nBuild v2, with edge cases."
        ),  # must refresh
    ...
    assert merged["stories"]["S1"]["agent_instructions"] == (
        "Preflight: test fixture — not a real plan\nBuild v2, with edge cases."
    )
