...
# ---------------------------------------------------------------------------
# Role provenance – resolve role configuration from plan, registry, env
# ---------------------------------------------------------------------------
...
# Import role_registry lazily to avoid import cycles.
import app.role_registry  # type: ignore
...
def resolve_role_provenance(
    ...
    *,
    ...
    """Resolve provenance for a single role.
    ...
    plan_role_config : dict | None
        Optional per‑role config from the current plan.
    registry : dict | None
        Optional model registry mapping.
    model_fallback : str | None
        Optional fallback model if none configured.
    environ : dict[str, str] | None
        Environment snapshot; used for env‑based provider overrides.

    Returns
    -------
    dict
        Dictionary containing keys:
        ``role``, ``provider``, ``model``, ``provider_source``, ``model_source``,
        ``restart_required`` and ``error``.
    """
    ...
    # Compute provider value for potential boundary case
    if plan_role_config and isinstance(plan_role_config.get(role), dict) and "provider" in plan_role_config[role]:
    ...
    try:
        ...
        # Boundary case: no model configured
