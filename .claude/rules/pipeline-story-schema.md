...
- `tdd_split` (runtime, written by the dispatcher — do NOT set it by hand in a plan file): a runtime marker the dispatcher sets to `true` once the test-author phase succeeds (read by the oracle tamper check at `pipeline/ci.py:428-442` to keep a TDD-split story's acceptance fixture strictly read-only); a re-ingest resets it to the plan's value.
- `files` (optional): exact repo-relative POSIX paths of the PRODUCTION files (including docs) the story may change. Test files are never listed and are always allowed. Absent means "not declared"; an empty list means no production file may change. `patch_story` cannot edit it — change the plan and re-ingest, which refreshes it (omitting it from the plan clears it). `ingest_plan` validates the value up front: it must be a list whose every entry is a non-empty `str` that does not start with `/`, contains no `\`, and has no `..` path component; duplicates are rejected.
